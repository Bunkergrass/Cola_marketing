            ...
            public void onClick(int position) {
                switch (position) {
                    case 0:
                        ...
                        // 指定调用相机拍照后照片的储存路径
                        Uri uri = FileProvider.getUriForFile(profileView.getBaseActivity(), profileView.getBaseActivity().getPackageName()+".provider",new File(dirPath, imageName));
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                case PHOTO_REQUEST_TAKEPHOTO:
                    Log.d("ProfilePrester"," dirPath:"+dirPath+"  imageName:"+imageName);
