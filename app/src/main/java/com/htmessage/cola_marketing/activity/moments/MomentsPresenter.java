...
import com.htmessage.cola_marketing.activity.moments.publish.MomentsPublishActivity;
...
public class MomentsPresenter implements MomentsContract.Presenter {
    ...
    public void loadeData(final int pageIndex) {
        ...
        new OkHttpUtils(context).post(params, HTConstant.URL_SOCIAL, new OkHttpUtils.HttpCallBack() {
            ...
            public void onResponse(JSONObject jsonObject) {
                Log.d("loadeData",jsonObject.toString());
    ...
    public void startToPhoto(int type) {
        if (!CommonUtils.isSdcardExist()) {
            Toast.makeText(monmentsView.getBaseContext(), R.string.sd_card_does_not_exist,Toast.LENGTH_SHORT).show();
            return;
        }

        cameraFile = new File(new HTPathUtils(null, monmentsView.getBaseContext()).getImagePath() + "/"
                + HTApp.getInstance().getUsername() + System.currentTimeMillis() + ".png");
        Uri uri = FileProvider.getUriForFile(monmentsView.getBaseContext(),monmentsView.getBaseContext().getPackageName()+".provider",cameraFile);
        if (type == 1){
    ...
    private void updateMomentBackGround(final String value){
        ...
        Luban.with(monmentsView.getBaseContext()).load(new File(value)).ignoreBy(100).filter(new CompressionPredicate() {
            @Override
            ...
            public void onError(Throwable e) {
                Toast.makeText(monmentsView.getBaseContext(),R.string.update_failed,Toast.LENGTH_SHORT).show();
            }
        }).launch();
    }
            ...
            public void onResponse(JSONObject jsonObject) {
                ...
                switch (code) {
                    case 1:
                        ...
                        Toast.makeText(monmentsView.getBaseContext(),R.string.update_success,Toast.LENGTH_SHORT).show();
                        break;
                    default:
                        Toast.makeText(monmentsView.getBaseContext(),R.string.update_failed,Toast.LENGTH_SHORT).show();
                        break;
