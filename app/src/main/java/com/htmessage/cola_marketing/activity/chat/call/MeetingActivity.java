...
import com.htmessage.cola_marketing.utils.TimeUtils;
...
public class MeetingActivity extends AppCompatActivity implements RTMeetHelper{
    ...
    private BroadcastReceiver receiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("3005")) {
                ...
                //setRes("聊天时长 "+new TimeUtils().getTimeExpend(startTime,endTime));
                Toast.makeText(getApplicationContext(), "通话已挂断", Toast.LENGTH_SHORT).show();
                finish();
            } else if (intent.getAction().equals("4004")) {
                videoTovoice();
            } else if (intent.getAction().equals("3004")) {
                setRes("已取消");
                Toast.makeText(getApplicationContext(), "对方正在通话中",Toast.LENGTH_SHORT).show();
                finish();
            } else if (intent.getAction().equals("5002")) {
                setRes("已取消");
                Toast.makeText(getApplicationContext(), "群视频通话呼叫被取消",Toast.LENGTH_SHORT).show();
                finish();
            }
        }
    };

    public void setRes(String extra){
    ...
    public void setSpeakerphoneOn(boolean on) {
        if (on) {
            ...
            //audioManager.setRouting(AudioManager.MODE_NORMAL, AudioManager.ROUTE_EARPIECE, AudioManager.ROUTE_ALL);//deprecated
            audioManager.setBluetoothScoOn(false);//关闭蓝牙SCO音频连接
            setVolumeControlStream(AudioManager.STREAM_VOICE_CALL);
    ...
    public Handler releaseHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1000) {
                ...
                timeOver();
                Toast.makeText(getApplicationContext(), "对方无应答",Toast.LENGTH_SHORT).show();
                finish();
            }
    ...
    public void OnBtnClicked(View btn) {
        switch (btn.getId()){
            case R.id.btn_hangup:
                ...
                if (isChating) {
                    endTime = TimeUtils.getCurrentTime();
                    s ="聊天时长 " + TimeUtils.getTimeExpend(startTime,endTime);
                    mMeetKit.Leave();
                }
                sendOverMessage();
                setRes(s);
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (action == 3003 || action == 4003) {
            Toast.makeText(getApplicationContext(), "通话已接通", Toast.LENGTH_SHORT).show();
...
//            }
    ...
    @Override
    public void OnRTCAudioActive(String s, int i) {
        MeetingActivity.this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                //每个图像的音频实时监测
