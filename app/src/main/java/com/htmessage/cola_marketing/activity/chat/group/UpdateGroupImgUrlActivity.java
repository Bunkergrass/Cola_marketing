...
public class UpdateGroupImgUrlActivity extends BaseActivity {
    ...
    private void initView() {
        ivGroup = this.findViewById(R.id.iv_group);

        tvSave = this.findViewById(R.id.btn_right);
        tvSave.setVisibility(View.VISIBLE);
        tvSave.setEnabled(false);
    }

    private void initData() {
        RequestOptions options = new RequestOptions().placeholder(R.drawable.default_group);
        Glide.with(getApplicationContext()).load(htGroup.getImgUrl()).apply(options).into(ivGroup);
...
//                        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(Environment.getExternalStorageDirectory(), imageName)));
                        Uri uri = FileProvider.getUriForFile(UpdateGroupImgUrlActivity.this,
                                getPackageName()+".provider", new File(Environment.getExternalStorageDirectory(), imageName));
