package com.htmessage.cola_marketing.activity.moments.widget;
...
public class MomentsItemView extends LinearLayout {
    ...
    private void init(Context context) {
        LayoutInflater.from(context).inflate(R.layout.widget_moments_item, this);
        ...
        ivPop = findViewById(R.id.iv_pop);
    ...
    public void initView(JSONObject json, String serverTime) {
        ...
        Glide.with(getContext()).load(avatar).apply(options).into(ivAvatar);
    ...
    private void showDeleteDialog(final String aId){
        AlertDialog dialog = new AlertDialog.Builder(getContext()).create();
        dialog.setTitle(R.string.confirm_delete_moment);
    ...
    public void updateGoodView(JSONArray data) {
       // Log.d("HomePagerFragment","---updateGoodView-----data:"+data.toString());
        goodArray.clear();
    ...
    private void initSingle(final String url, LinearLayout linearLayout) {
        ...
        final ImageView ivSingle= view.findViewById(R.id.iv_single_moments);
...
//        imageView.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        ...
        Glide.with(getContext()).asBitmap().load(HTConstant.baseImgUrl + url).apply(options).into(ivSingle);
    ...
    private void initFour(final ArrayList<String> images, LinearLayout mainLinearLayout) {
        ...
        for (int i = 0; i < lines; i++) {
            ...
            for (int n = i * numColumns; n < (numColumns * (i + 1)); n++) {
                ...
                linearLayout.addView(imageView, new LayoutParams(0, LayoutParams.WRAP_CONTENT, (float) 1));
                RequestOptions options = new RequestOptions().diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.default_image2).centerCrop();
                Glide.with(getContext()).load(HTConstant.baseImgUrl + url).apply(options).into(imageView);
                final int finalN = n;
    ...
    private class TextClickableSpan extends ClickableSpan {
        ...
        public void updateDrawState(TextPaint ds) {
            if (type == 0) {
                ds.setColor(getResources().getColor(R.color.text_color_mid));
    ...
    private void showMorePop(ImageView imageView, final String aid) {
        ...
        if (mMorePopupWindow == null) {
            ...
            tvPopGood = conentView.findViewById(R.id.tv_good);
